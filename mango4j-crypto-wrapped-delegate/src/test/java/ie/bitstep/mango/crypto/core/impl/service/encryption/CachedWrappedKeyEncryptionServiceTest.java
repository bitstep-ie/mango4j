...
import ie.bitstep.mango.crypto.core.impl.service.encryption.CachedWrappedKeyEncryptionService;
...
class CachedWrappedKeyEncryptionServiceTest implements CryptoKeyProvider {
	...
	EncryptionService setUp(int keySize) {
		...
		cryptoKey.setType(WrappedCryptoKeyTypes.CACHED_WRAPPED.getName());
		cryptoKey.setUsage(CryptoKeyUsage.ENCRYPTION);
		cryptoKey.setKeyStartTime(Instant.now());
	...
	void testCachedWrappedKeyEncryptionService() {
		...
		// WHEN
		CachedWrappedKeyEncryptionService cachedWrappedKeyEncryptionService = new CachedWrappedKeyEncryptionService(
	...
	void initCipherModeNone() throws NoSuchPaddingException, NoSuchAlgorithmException {
		...
		assertDoesNotThrow(() ->
				CipherManager.initCipher(
						...
						new byte[0],
						cipher,
						CachedWrappedKeyEncryptionService.generateDataEncryptionKey(Generators.generateRandomBits(256), cipherConfig)
				)
		);
	}
	...
	void multiEncryptDecrypt() {
		EncryptionService encryptionService = setUp(256);
		CiphertextContainer encrypted1 = encryptionService.encrypt(getById("AES-GCM"), data);
		CiphertextContainer encrypted2 = encryptionService.encrypt(getById("AES-GCM"), data);
		...
		assertThat(encrypted1.getCryptoKey()).isEqualTo(getById("AES-GCM"));
		assertThat(encrypted2.getCryptoKey()).isEqualTo(getById("AES-GCM"));
